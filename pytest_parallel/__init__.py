...
def pytest_addoption(parser):
    ...
    parser.addini('serial_class_execution', serial_class_execution_help, type="bool", default=False)
...
class ParallelRunner(object):
    ...
    def pytest_runtestloop(self, session):
        ...
        if serial_class_execution:
            ...
            instance_to_class_test_item_indices: Dict[Instance, Set[int]] = defaultdict(set)
            for i, item in enumerate(session.items):
                if isinstance(item.parent, Module):
                    function_test_item_indices.add(i)
                elif isinstance(item.parent, Instance) and isinstance(item.parent.parent, Class):
                    instance_to_class_test_item_indices[item.parent].add(i)
                else:
                    raise TypeError(f"Test item was not child of module or class, was {type(item.parent)}")
